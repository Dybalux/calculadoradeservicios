        ...
        try {
            ...
            if (error) {
                ...
                const nextEventsList = events
                    ...
                    .slice(0, 5); // Aumente a 5 para que veas mas proximos
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto space-y-8">
            <div className="flex justify-between items-center">
                ...
                <Link to="/calculadoradeservicios" className="inline-block bg-blue-500 hover:bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg text-sm transition-colors">
                    Nuevo Presupuesto
                </Link>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                {/* --- SECCION PROXIMOS EVENTOS (MEJORADA) --- */}
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4">Proximos Eventos</h3>
                    {stats.nextEvents.length === 0 ? (
                        <p className="text-gray-500">No hay eventos proximos agendados.</p>
                    ) : (
                        <ul className="space-y-4">
                            {stats.nextEvents.map(event => {
                                // Calculos individuales para cada fila
                                const info = event.client_info || {};
                                const total = parseFloat(info.total || 0);
                                const advance = parseFloat(info.advance || 0);
                    ...
                    <div className="mt-4 text-right">
                        <Link to="/eventos" className="inline-block bg-blue-500 hover:bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg text-sm transition-colors">Ver Agenda Completa</Link>
                    </div>
                </div>
            </div>
        </div>
